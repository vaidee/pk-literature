...
import type { CatalogMediaAssetTable } from "../database/database.types";
...
// opposed to CatalogMediaAssetTable itself, which is typed for insert/update.
export function toMediaAsset(
  row: Selectable<Pick<CatalogMediaAssetTable, "id" | "assetType" | "s3Key" | "widthPx" | "heightPx">> | null,
): MediaAsset | null {
  if (!row) return null;
  return {
    id: row.id,
